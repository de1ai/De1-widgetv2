...
import type { ChainType, CoinKey } from '@de1/widget-types'

/**
 * Get a quote for a token transfer
 * @param params - The configuration of the requested quote
 * @param options - Request options
 * @throws {De1Error} - Throws a De1Error if request fails
...
export const getContractCallsQuote = async (
  ...
  params.preferExchanges ??= _config.routeOptions?.exchanges?.prefer
  // send request
  return await request<De1Step>(`${_config.apiUrl}/quote/contractCalls`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const response = await withDedupe(
    () =>
      request<ChainsResponse>(
        `https://open-api.de1.exchange/v3/widgetv2/chains`
      ),
    { id: `${getChains.name}` }
  )
...
export const getGasRecommendation = async (
  ...
  if (params.chainId === 20000000000006) {
    return {
      available: false,
