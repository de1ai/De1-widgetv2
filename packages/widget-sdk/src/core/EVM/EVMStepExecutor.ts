...
import { De1ErrorCode } from '../../errors/constants.js'
...
export class EVMStepExecutor extends BaseStepExecutor {
  ...
  checkClient = async (step: De1StepExtended, process?: Process) => {
    ...
    if (
      ...
      this.statusManager.updateProcess(step, processToUpdate.type, 'FAILED', {
        error: {
          code: De1ErrorCode.WalletChangedDuringExecution,
      ...
      throw await parseEVMErrors(
        new TransactionError(
          De1ErrorCode.WalletChangedDuringExecution,
          errorMessage
        ),
        step,
        process
    ...
    try {
      ...
      if (!step.transactionRequest) {
        throw new TransactionError(
          De1ErrorCode.TransactionUnprepared,
