import type { ChainId, De1Step } from '@de1/widget-types'
...
export class StatusManager {
   ...
   */
  initExecutionObject = (step: De1StepExtended): Execution => {
    if (!step.execution) {
      step.execution = {
        status: 'PENDING',
        process: [],
   ...
   */
  removeProcess = (step: De1StepExtended, type: ProcessType): void => {
    if (!step.execution) {
      throw new Error("Execution hasn't been initialized.")
    }
  ...
  updateStepInRoute = (step: De1Step): De1Step => {
