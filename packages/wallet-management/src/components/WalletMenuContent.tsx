...
import type { WalletMenuOpenOptions } from '../providers/WalletMenuProvider/types.js'
...
export const WalletMenuContent: React.FC<WalletMenuContentProps> = ({
  ...
  const filteredInstalledWallets = useMemo(() => {
    ...
    return installedWallets
      .map((wallet) => ({
        ...wallet,
        connectors: wallet.connectors.filter(
          ({ chainType }) => {
            if (wallet.name.toLowerCase().includes('phantom') && options?.chainType === ChainType.UTXO) {
              return false
            }
            return chainType === options?.chainType
          }
        ),
      }))
      .filter((wallet) => wallet.connectors.length > 0)
  }, [installedWallets, options?.chainType])
    ...
    <>
      ...
      <DialogContent sx={{ padding: 2 }}>
        ...
        >
          ...
          >
            <List sx={{ padding: 0 }}>
              {filteredInstalledWallets.map(({ id, name, icon, connectors }) => {
                if (connectors.length === 1) {
                  const { chainType, connector } = connectors[0]
                  return getWalletButton(id, name, chainType, connector, false, icon)
                }
                return (
                  <ListItemButton
                    key={name}
