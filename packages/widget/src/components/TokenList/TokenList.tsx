...
export const TokenList: FC<TokenListProps> = ({
  ...
  const allTokens = (tokensWithBalance ?? chainTokens ?? []) as TokenAmount[]
  let filteredTokens = filterTokensBySource(
    allTokens,
    tokenSourceFilter
  )
  ...
  const searchedTokenMatchesFilter =
    !!searchedToken &&
  ...
  useEffect(() => {
    if (selectedChain?.chainType && accounts.length > 0) {
      ...
      const matchingAccount = accounts.find(
        (acc) => acc.chainType === selectedChain.chainType && acc.isConnected
      )
