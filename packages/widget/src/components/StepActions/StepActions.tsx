...
import type { De1Step, StepExtended } from '@de1/widget-sdk'
    ...
    <Box {...other}>
      ...
      >
        <Badge
          ...
          badgeContent={<SmallAvatar src={De1ToolLogo} />}
        >
          <StepAvatar
            variant="circular"
            src={toolDetails.logoURI ?? De1ToolLogo}
        ...
        >
          ...
          >
            {toolDetails.name?.includes('De¹ Exchange')
              ? toolDetails.name
              : t('main.stepDetails', {
...
export const CustomStepDetailsLabel: React.FC<StepDetailsLabelProps> = ({
  ...
  const toolDetails =
    subvariant === 'custom' &&
    (step as unknown as De1Step).includedSteps?.length > 0
      ? (step as unknown as De1Step).includedSteps.find(
          (step) => step.tool === 'custom' && step.toolDetails.key !== 'custom'
        )?.toolDetails || step.toolDetails
      : step.toolDetails
