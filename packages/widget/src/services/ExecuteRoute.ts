...
function isValidApprovalSpender(address?: string | null): boolean {
  if (!address) {
    return false
  }
...
async function swapQuoteMEV(
  ...
  try {
    ...
    const DE1_CONTRACT = new ethers.Contract(
    ...
    const oldCallData = DE1_CONTRACT.interface.decodeFunctionData(
      'swap',
    ...
    const minOutAmounts = await Promise.all(
      [1, 2, 3].map(async (i) => {
        ...
        const params = {
          ...
          data: DE1_CONTRACT.interface.encodeFunctionData(
            'swap',
            callData
          ) as `0x${string}`,
    ...
    const finalCallData = DE1_CONTRACT.interface.encodeFunctionData(
      'swap',
      callData
    )
...
interface ExecuteRouteOptions {
  ...
  solanaWallet?: any
  // Near wallet-selector instance is passed from the React layer via options to avoid calling Hooks here
  nearWallet?: any
}

interface ExtendedDe1Step extends De1Step {
  quoteData?: any
  execution?: {
    ...
    if (
      ...
      if (allowance < BigInt(amount)) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            verifiedAllowance = (await (publicClient as any).readContract({
              address: step.action.fromToken.address as `0x${string}`,
              abi: [
                {
                  constant: true,
            ...
            console.warn(
              `[approve-verify] Failed to re-read allowance (attempt ${attempt + 1}/${maxRetries + 1}):`,
              readError
            )
