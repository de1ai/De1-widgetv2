...
import type { SDKProvider } from '@de1/widget-sdk'
import {
  ChainType,
  Near,
  EVM,
...
export const SDKProviders = () => {
  ...
  const accountSourceConfig =
    walletConfig?.externalWagmiConfig ?? wagmiContextConfig
  ...
  useEffect(() => {
    ...
    if (!hasConfiguredEVMProvider && accountSourceConfig) {
      providers.push(
        EVM({
          getWalletClient: () =>
            getWagmiConnectorClient(accountSourceConfig),
