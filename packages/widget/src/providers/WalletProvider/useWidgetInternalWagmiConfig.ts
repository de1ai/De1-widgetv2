 ...
 */
export function useWidgetInternalWagmiConfig(
  options: UseWidgetInternalWagmiConfigOptions = {}
): UseWidgetInternalWagmiConfigResult {
  const { walletConfig } = options
  const { chains } = useAvailableChains()

  const ref = useRef<DefaultWagmiConfigResult | null>(null)
  if (!ref.current) {
