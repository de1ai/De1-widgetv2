...
import type { AllowDeny, WidgetBridgesConfig } from '../types/widget.js'
...
export async function getCrossChainQuote({
  ...
  try {
    ...
    const adapters = registry.getAllAdapters().filter((adapter) => {
      if (
        !isBridgeAdapterEnabled(adapter, disabledBridges, bridgesConfig)
      ) {
        return false
      }
...
export const bridgeExecuteSwap = async ({
  ...
  // Near wallet-selector instance for the Near Intents adapter; other adapters ignore it
  nearWallet?: any
}) => {
