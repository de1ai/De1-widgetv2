...
export class NearIntentsAdapter extends BaseSwapAdapter {
  ...
  constructor() {
    ...
    OpenAPI.BASE = 'https://1click.de1.exchange'
  ...
  async executeSwap(
    ...
    if (quote.quoteParams.fromChain === NonEvmChain.Near) {
      return new Promise<NormalizedTxResponse>(async (resolve, reject) => {
        ...
        const amount = String(rawAmount) // yoctoNEAR string
        ...
        // Standard wrap.near contract
        const WRAP_CONTRACT_ID = 'wrap.near'

        const tokenContract = fromToken.address as string | undefined

        if (isNative) {
          // Native NEAR: storage_deposit the bridge on wNEAR, wrap via near_deposit, then ft_transfer to the bridge
          transactions.push({
            signerId: nearWallet.signedAccountId,
            receiverId: WRAP_CONTRACT_ID,
            actions: [
              {
                // 1) storage_deposit so the bridge account is registered on wNEAR
                type: 'FunctionCall',
                params: {
              ...
              {
                // 2) near_deposit: wrap native NEAR into wNEAR
                type: 'FunctionCall',
                params: {
                  methodName: 'near_deposit',
                  args: {},
                  gas: '30000000000000',
                  deposit: amount, // original NEAR amount in yoctoNEAR
                },
              },
              {
                // 3) ft_transfer: send wNEAR to the bridge address
                type: 'FunctionCall',
                params: {
                  ...
                  gas: '30000000000000',
                  deposit: '1', // NEP-141 requires 1 yoctoNEAR
                },
              },
            ],
          })
        } else if (tokenContract) {
          // Non-native NEP-141 token: storage_deposit the bridge on the token contract, then ft_transfer
          transactions.push({
            signerId: nearWallet.signedAccountId,
        ...
        // MyNearWallet redirects to a web page; persist the tx locally first
