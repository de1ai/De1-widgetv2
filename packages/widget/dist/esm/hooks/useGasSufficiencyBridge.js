...
export const useGasSufficiencyBridge = (route) => {
    ...
    const { data: insufficientBridge, isLoading } = useQuery({
        ...
        queryFn: async ({ queryKey: [, accountAddress] }) => {
            ...
            if (route.fromChainId !== route.toChainId) {
                const prependedOperatingExpenseCost = route.data.prependedOperatingExpenseCost || 0n;
                let gas = BigInt(Number.parseInt((1.1 * Number(prependedOperatingExpenseCost)).toString()));
...
//# sourceMappingURL=useGasSufficiencyBridge.js.map