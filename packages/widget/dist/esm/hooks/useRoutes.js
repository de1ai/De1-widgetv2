...
import { ChainType, De1ErrorCode } from '@de1/widget-sdk';
...
import { useWidgetExecutionWagmiConfig } from './useWidgetExecutionWagmiConfig.js';
const refetchTime = 60000;
export const useRoutes = ({ observableRoute } = {}) => {
    const wagmiConfig = useWidgetExecutionWagmiConfig();
    const { wallet: solanaWallet } = useWallet();
    const { subvariant, sdkConfig, fee, feeConfig, evmReferrer, solanaReferrer, referrer, bridges } = useWidgetConfig();
    ...
    const isSvmFromChain = fromChain?.chainType === ChainType.SVM;
    const effectiveEvmReferrer = evmReferrer ?? referrer;
    const activeReferrer = isSvmFromChain ? solanaReferrer : effectiveEvmReferrer;
    const activeReferrerAddress = activeReferrer?.address || '';
    const activeReferrerFee = activeReferrer?.fee || '';
    ...
    const { data, isLoading, isFetching, isFetched, dataUpdatedAt, refetch } = useQuery({
        ...
        queryFn: async ({ queryKey: [_, fromAddress, fromChainId, fromTokenAddress, fromTokenAmount, toAddress, toChainId, toTokenAddress, toTokenAmount, contractCalls, slippage = defaultSlippage, swapOnly, disabledBridges, disabledExchanges, allowedBridges, allowedExchanges, routePriority, subvariant, allowSwitchChain, enabledRefuel, gasRecommendationFromAmount, fee, referrerAddress, referrerFee, isBatchingSupported, _observableRouteId,], signal, }) => {
            try {
                ...
                if (fromChainId !== toChainId) {
                    ...
                    // Use De1Service for same-chain swaps
                    if (account.address) {
                        quoteResult = await De1Service.getSwapQuote({
                            ...
                            account: account.address,
                            slippage: formattedSlippage, // De¹ Exchange expects slippage like '100' for 1%
                        ...
                        quoteResult = await De1Service.getQuote({
                            chain: fromChainId.toString(),
                            inTokenSymbol: fromToken?.symbol || '',
                            inTokenAddress: fromTokenAddress,
                            outTokenSymbol: toToken?.symbol || '',
                ...
                // minOutAmount calculation is now handled within DebridgeService or De1Service
                    ...
                    steps: [
                        {
                            ...
                            toolDetails: {
                                key: isBridge ? data?.quoteAdapterKey : 'de1',
                                name: isBridge ? data?.quoteAdapterName : 'De¹ Exchange',
                                logoURI: isBridge
                                    ? 'https://s3.de1.exchange/static/debridge.svg'
                                    : De1ToolLogo,
                            },
                            action: {
                                fromChainId: fromChainId,
                ...
                console.error('Failed to fetch De¹ Exchange routes:', error);
        ...
        retry(failureCount, error) {
            ...
            if (error?.code === De1ErrorCode.NotFound) {
                return false;
            }
            return false;
        },
