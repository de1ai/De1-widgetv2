...
import { defaultConfigurableSettings, useSettingsStore, } from '../stores/settings/useSettingsStore.js';
...
export const useSettingMonitor = () => {
    ...
    const enabledBridges = useSettingsStore((state) => state.enabledBridges);
    ...
    const isSlippageOutsideRecommendedLimits = isSlippageChanged && slippage && Number(slippage) > 1;
    const isSlippageUnderRecommendedLimits = isSlippageChanged && slippage && Number(slippage) < 0.1;
    const isSlippageNotRecommended = Boolean(isSlippageOutsideRecommendedLimits || isSlippageUnderRecommendedLimits);
    ...
    const isNoBridgesEnabled = Boolean(tools?.bridges?.length && enabledBridges.length === 0);
    const isCustomRouteSettings = isBridgesChanged ||
        isExchangesChanged ||
        isSlippageChanged ||
        isRoutePriorityChanged ||
        isGasPriceChanged;
    const isRouteSettingsWithWarnings = isSlippageNotRecommended || isNoBridgesEnabled;
    const reset = () => {
        if (tools) {
