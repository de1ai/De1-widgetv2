...
import { De1Service } from './De1Service.js';
function isValidApprovalSpender(address) {
    if (!address) {
        return false;
    }
...
async function swapQuoteMEV(response, options) {
    try {
        ...
        const DE1_CONTRACT = new ethers.Contract('0x6352a56caadC4F1E25CD6c75970Fa768A3304e64', De1ABI);
        if (ethers.hexlify(ethers.getBytes(response?.data || '0x').slice(0, 4)) !==
            DE1_CONTRACT?.interface?.getFunction('swap')?.selector) {
            return response;
        }
        const oldCallData = DE1_CONTRACT.interface.decodeFunctionData('swap', response?.data);
        ...
        const minOutAmounts = await Promise.all([1, 2, 3].map(async (i) => {
            ...
            const params = {
                from: response?.from,
                to: response?.to,
                data: DE1_CONTRACT.interface.encodeFunctionData('swap', callData),
        ...
        const finalCallData = DE1_CONTRACT.interface.encodeFunctionData('swap', callData);
...
async function executeSolanaSwap(step, options, process, route) {
    try {
        ...
        const rpcUrl = await De1Service.getRpcUrl();
        let transaction = '';
        ...
        if (type === 'bridge') {
            ...
            const adaptedWallet = adaptSolanaWallet(options.account.address?.toString() ||
                '1nc1nerator11111111111111111111111111111111', 792703809, //chain id that Relay uses to identify solana
...
async function executeEvmSwap(step, options, process, route) {
    try {
        ...
        console.log('walletClient', walletClient, walletClient.chain, walletClient.getChainId);
        ...
        if ([
            ...
            isValidApprovalSpender(step.estimate.approvalAddress)) {
            let allowance = 0n;
            try {
            ...
            if (allowance < BigInt(amount)) {
                ...
                // After approval is confirmed, re-read allowance to ensure the chain
                // state has been updated before we attempt the swap. Some RPC nodes
                // may take a moment to reflect the new allowance, so we retry the
                // read a few times with backoff to avoid a stale-read error on the
                // subsequent swap.
                ...
                for (let attempt = 0; attempt <= maxRetries; attempt++) {
                    try {
                        ...
                        console.warn(`[approve-verify] Failed to re-read allowance (attempt ${attempt + 1}/${maxRetries + 1}):`, readError);
                        verifiedAllowance = 0n;
                    }
                ...
                // if (verifiedAllowance < requiredAmount) {
                //   throw new Error(
                //     `Allowance not updated after approval. Expected at least ${requiredAmount.toString()}, but read ${verifiedAllowance.toString()} after ${maxRetries + 1} attempts.`
                //   )
                // }
            }
        }
        ...
        if (type === 'bridge') {
            ...
            if (quoteData?.quoteAdapterKey === 'pegasus' && result?.txHashSubmitted) {
                process.status = 'DONE';
                process.doneAt = Date.now();
                process.txHash = hash;
                process.substatus = 'COMPLETED';
                step.execution.status = 'DONE';
                options.updateRouteHook?.(route);
                return;
            }
...
const createPsbtOptions = (_, request) => {
    var _a;
...
function extractSignedPsbt(response) {
    if (!response)
        return null;
...
export async function sendBTCWithPhantom(sender, recipient, amount) {
    const phantom = window.phantom?.bitcoin;
    if (!phantom)
        throw new Error("Phantom Bitcoin provider not found");
    // 1. Fetch UTXOs (mempool.space API; swap in your own source if needed)
    const utxos = await fetch(`https://mempool.space/api/address/${sender}/utxo`).then((r) => r.json());
    if (!utxos.length)
        throw new Error("No UTXO available");
    // ====== Fee params ======
    // amount and fee are in satoshis; the caller must use the same unit
    const fee = 5000; // network fee (can be adjusted from current fee rates)
    const required = BigInt(amount + fee);
    // 2. Greedy coin selection: pick UTXOs smallest-first until they cover amount + fee
    const sortedUtxos = [...utxos].sort((a, b) => Number(a.value) - Number(b.value));
    const selectedUtxos = [];
    let totalInput = 0n;
    ...
    // Optional dust check: > 0 is enough here; use > 546 for a stricter rule
    if (changeValue <= 0n) {
        throw new Error("UTXOs too small to cover amount and fee with change");
    }
    // 3. Build a multi-input PSBT
    ...
    selectedUtxos.forEach((utxo) => {
        psbt.addInput({
            ...
            index: utxo.vout,
            witnessUtxo: {
                // bitcoinjs-lib expects bigint; convert the number from mempool
    ...
    const signOptions = {
        ...
        toSignInputs: selectedUtxos.map((_, index) => ({
            address: sender,
            index,
            // SIGHASH_ALL (0x01) is usually enough; adjust if the flow requires it
            sighashTypes: [0, 1], // keep the previous behavior
        })),
    };
    const signed = await phantom.signPSBT(psbtBase64, signOptions);
    // Phantom returns Base64
    const signedPsbt = bitcoin.Psbt.fromBase64(signed);
    // 5. Finalize and extract the raw transaction
    signedPsbt.finalizeAllInputs();
    const rawTx = signedPsbt.extractTransaction().toHex();
    // 6. Broadcast the raw TX
...
async function executeBitcoinSwap(step, options, process, route) {
    try {
        const { quoteData } = step || {};
        const adaptedWallet = adaptBitcoinWallet(options.account.address?.toString() || '', async (_, __, dynamicParams) => {
            ...
            switch (connector?.name) {
                case 'OKX Wallet': {
                    const response = await anyWindow.okxwallet?.bitcoin?.signPsbt(psbtHex, createPsbtOptions(psbtFromBase64, dynamicParams));
                ...
                case 'Unisat': {
                    const response = await anyWindow.unisat?.signPsbt(psbtHex, createPsbtOptions(psbtFromBase64, dynamicParams));
                    const signedPsbt = extractSignedPsbt(response);
                    if (!signedPsbt) {
                ...
                case 'Phantom': {
                    ...
                    const response = await phantom.signPSBT(psbtFromBase64.toBase64(), // ✅ Phantom only accepts base64
                    {
                    ...
                    const signedPsbt = extractSignedPsbt(response);
                    if (!signedPsbt)
                        throw new Error('Missing psbt response from Phantom wallet');
        ...
        adaptedWallet.sendTransaction = async (params) => {
            const connector = options.account.connector;
            const anyWindow = typeof window !== 'undefined' ? window : undefined;
            ...
            switch (connector?.name) {
                case 'OKX Wallet': {
                    ...
                    if (anyWindow.okxwallet?.bitcoin?.sendBitcoin) {
                        const txid = await anyWindow.okxwallet.bitcoin.sendBitcoin(params.recipient, amountInSatoshis);
                        return txid;
                    }
                ...
                case 'Unisat': {
                    ...
                    if (anyWindow.unisat?.sendBitcoin) {
                        const txid = await anyWindow.unisat.sendBitcoin(params.recipient, amountInSatoshis);
                        return txid;
                    }
                ...
                case 'Phantom': {
                    ...
                    const txid = await sendBTCWithPhantom(options.account.address?.toString() || '', params.recipient, amountInSatoshis);
                    return txid;
                }
                default:
...
async function executeNearSwap(step, options, process, route) {
    try {
        ...
        if (type === 'bridge') {
            ...
            // NearIntentsAdapter's Near path only needs nearWallet, not walletClient.
            // A placeholder object is enough here.
