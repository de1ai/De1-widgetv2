...
const initialiseDefaultValues = (defaultValues, fromAmount, toAmount, toAddress, hiddenToAddress) => ({
    ...formDefaultValues,
    ...defaultValues,
    fromAmount: (typeof fromAmount === 'number' ? fromAmount?.toPrecision() : fromAmount) ||
        formDefaultValues.fromAmount,
    toAmount: (typeof toAmount === 'number' ? toAmount?.toPrecision() : toAmount) ||
...
export const FormStoreProvider = ({ children, formRef, }) => {
    const widgetConfig = useWidgetConfig();
    const { fromChain, fromToken, toChain, toToken, fromAmount, toAmount, toAddress, hiddenUI, formUpdateKey, } = widgetConfig;
    ...
    const reactiveFormValues = useMemo(() => ({
        ...
        ...(configHasToAmount
            ? {
                toAmount: (typeof toAmount === 'number'
    ...
    if (!storeRef.current) {
        storeRef.current = createFormStore(initialiseDefaultValues(reactiveFormValues, fromAmount, toAmount, toAddress, hiddenToAddress));
    }
