...
import type { BaseToken, ChainType, ContractCall, Order, RouteOptions, SDKConfig, StaticToken, Token } from '@de1/widget-sdk';
...
export type WidgetSubvariant = 'default' | 'swap' | 'bridge' | 'split' | 'custom' | 'refuel';
...
export type WidgetThemeComponents = Pick<Components<Theme>, 'MuiAppBar' | 'MuiAvatar' | 'MuiButton' | 'MuiCard' | 'MuiIconButton' | 'MuiInputCard' | 'MuiTabs' | 'MuiTypography'>;
...
export interface FieldValues extends Omit<DefaultFieldValues, 'fromAmount' | 'toAmount' | 'toAddress'> {
    fromAmount?: number | string;
    toAmount?: number | string;
