...
export function useExternalWalletProvider() {
    ...
    const data = useMemo(() => {
        ...
        const hasExternalEVM = hasExternalEVMContext || Boolean(walletConfig?.externalWagmiConfig);
        const providers = [];
        if (hasExternalEVM) {
        ...
        const hasExternalProvider = hasExternalEVM || hasExternalSVMContext || hasExternalUTXOContext;
        const usePartialWalletManagement = walletConfig?.usePartialWalletManagement === true;
        ...
        if (hideEvmWalletMenu && !usePartialWalletManagement) {
            internalChainTypes = internalChainTypes.filter((chainType) => chainType !== ChainType.EVM);
        }
        const useExternalWalletProvidersOnly = hasExternalProvider && !usePartialWalletManagement;
        return {
