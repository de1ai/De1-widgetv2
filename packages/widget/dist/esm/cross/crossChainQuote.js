import { isBridgeAdapterEnabled } from './bridgeTools.js';
 ...
 */
export async function getCrossChainQuote({ fromMsg, toMsg, inAmount, slippage_tolerance, account, recipient, tokenInUsd = 0, tokenOutUsd = 0, feeBps = 10, walletClient, publicClient, publicKey, nearTokens = [], disabledBridges = [], bridgesConfig, }) {
    try {
        ...
        const adapters = registry.getAllAdapters().filter((adapter) => {
            if (!isBridgeAdapterEnabled(adapter, disabledBridges, bridgesConfig)) {
                return false;
            }
        ...
        return {
            data: {
                ...
                from: account,
                to: best.quote.contractAddress || ZERO_ADDRESS,
                ...
                data: best.quote.rawQuote.txParams?.data ||
                    ...
                    '0x',
                approveContract: best.quote.contractAddress || ZERO_ADDRESS,
...
export const ADAPTER_LOGIN_URLS = {
    ...
    rhea: 'https://boss.rhea.finance/',
    xyfinance: 'https://xy.finance/',
    kyberswap: 'https://kyberswap.com/',
    pegasus: 'https://open-api.de1.exchange/pegasusfi',
};
