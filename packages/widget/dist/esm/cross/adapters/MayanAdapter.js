...
export class MayanAdapter extends BaseSwapAdapter {
    ...
    async executeSwap({ quote }, walletClient) {
        ...
        if (fromChain === 'solana') {
            ...
            // Gasless Swift requires EIP-712 signing + submitSwiftEvmSwap, not a regular sendTransaction (see SDK swapFromEvm)
            if (mayanQuote.type === 'SWIFT' && mayanQuote.gasless) {
                throw new Error('Mayan gasless Swift orders must use @mayanfinance/swap-sdk swapFromEvm (EIP-712 + order submit). This adapter only supports on-chain transaction payloads.');
            }
            const mayanApiKey = getMayanApiKey();
            // v13+ getSwapFromEvmTxPayload is async; referrerAddresses must be provided per network type (docs)
