import { AggregateDexRouter, BitgetRouter, NearSmartRouter, completeQuote, convertSlippageToBasisPoints, normalizeEvmAddress, normalizeTokenId, } from '@rhea-finance/cross-chain-aggregation-dex';
...
const SUPPORTED_RHEA_EVM_CHAINS = [
    ChainId.ETH,
    ChainId.BSC,
    ChainId.ARB,
    ChainId.BAS,
...
const SUPPORTED_RHEA_CHAINS = [NonEvmChain.Near, ...SUPPORTED_RHEA_EVM_CHAINS];
...
function getBitgetApiConfig() {
    const apiKey = readEnvValue('RHEA_BITGET_API_KEY') ||
        readEnvValue('BITGET_API_KEY') ||
        readEnvValue('BGW_API_KEY');
    const apiSecret = readEnvValue('RHEA_BITGET_API_SECRET') ||
        readEnvValue('BITGET_API_SECRET') ||
        readEnvValue('BGW_API_SECRET');
    const baseUrl = readEnvValue('RHEA_BITGET_API_BASE') ||
...
function toBitgetNativeContract(address) {
...
function getBitgetChainIdentifier(chain) {
...
function getRheaEvmChainName(chain) {
...
let oneClickTokensPromise;
function getOneClickBlockchain(chain) {
...
async function getOneClickTokens() {
    oneClickTokensPromise || (oneClickTokensPromise = fetch(`${OpenAPI.BASE}/v0/tokens`, {
...
async function resolveOneClickAssetId(token, chain) {
    ...
    const match = supportedTokens.find(item => item.blockchain === blockchain &&
        (native
...
async function callBitgetApi(params) {
    const { apiKey, apiSecret, baseUrl } = getBitgetApiConfig();
    if (!apiKey || !apiSecret) {
        throw new Error('Bitget API credentials are missing. Set RHEA_BITGET_API_KEY and RHEA_BITGET_API_SECRET.');
    }
    ...
    const response = await fetch(`${baseUrl}${params.path}${searchParams.toString() ? `?${searchParams.toString()}` : ''}`, {
        method: 'POST',
        headers: {
...
function normalizeBitgetQuoteData(data) {
    ...
    return {
        ...
        estimateRevert: data?.estimateRevert ??
            quoteData?.estimateRevert ??
            false,
    };
}
function normalizeBitgetSwapData(data) {
...
function getCurrencyAddress(token) {
...
function isNearChain(chain) {
    return chain === NonEvmChain.Near || chain === 'near';
}
function isSupportedBridgePair(fromChain, toChain) {
    if (fromChain === toChain) {
        return false;
    }
    return SUPPORTED_RHEA_CHAINS.includes(fromChain) && SUPPORTED_RHEA_CHAINS.includes(toChain);
}
function toEvmToken(token) {
...
function buildBluechipTokens(sourceKind, sourceToken, chainId) {
    if (sourceKind === 'near') {
        return {
            NEAR: {
    ...
    const nativeCurrency = chainId !== undefined ? NativeCurrencies[chainId] : undefined;
    const sourceIsNative = isNativeEvmToken(sourceToken);
    const nativeWrappedAddress = sourceIsNative
        ? RHEA_NATIVE_EVM_ADDRESS
        : nativeCurrency?.wrapped?.address || RHEA_NATIVE_EVM_ADDRESS;
    const nativeWrappedSymbol = nativeCurrency?.wrapped?.symbol || 'WETH';
    const bluechips = {
        ETH: {
            ...
            assetId: sourceIsNative
                ? `evm-${getRheaEvmChainName(chainId) || 'ethereum'}-native`
...
function buildFindPathAdapter() {
    return {
        async findPath(params) {
            const response = await fetch(`${RHEA_FIND_PATH_URL}/findPath?${new URLSearchParams({
...
function buildSwapMultiDexPathAdapter() {
    return {
        async swapMultiDexPath(params) {
            const response = await fetch(`${RHEA_SMARTX_URL}/swapMultiDexPath?${new URLSearchParams({
                amountIn: params.amountIn,
                tokenIn: params.tokenIn,
                tokenOut: params.tokenOut,
                slippage: String(params.slippage),
...
function buildIntentsQuotationAdapter() {
    return {
        async quote(params) {
            const quoteRequest = {
            ...
            const result = (await OneClickService.getQuote(quoteRequest));
            const quote = result?.quoteSuccessResult?.quote || result?.quote || result;
...
function buildQuoteRouters(sourceKind, chainId, publicClient) {
    if (sourceKind === 'near') {
        return [
            new NearSmartRouter({
...
function buildExecutionRouters(sourceKind, chainId, walletClient, nearWallet) {
    if (sourceKind === 'near') {
        return [
            new NearSmartRouter({
                findPathAdapter: buildFindPathAdapter(),
...
function buildQuoteOnlyNearChainAdapter() {
    return {
        async call() {
...
function buildQuoteOnlyEvmChainAdapter(publicClient) {
    return {
...
function buildNearChainAdapter(nearWallet) {
    return {
        async call({ transactions }) {
            ...
            return {
                ...
                txHashArr: Array.isArray(txResult)
                    ? txResult.map((item) => item?.transaction?.hash || item?.hash).filter(Boolean)
                    : hash
                        ? [hash]
                        : [],
            };
...
function buildEvmChainAdapter(walletClient) {
    return {
...
function buildBitgetAdapter() {
    return {
        async quote(params) {
            ...
            const response = await callBitgetApi({
...
async function executeDirectBridgeTransfer(params) {
    ...
    if (sourceKind === 'evm') {
        ...
        const isNative = Boolean(sourceToken.isNative) || tokenAddress === '';
        if (isNative) {
    ...
    const isNative = getCurrencyAddress(sourceToken) === RHEA_NATIVE_NEAR ||
...
export class RheaAdapter extends BaseSwapAdapter {
    ...
    async getQuote(params) {
        try {
            ...
            const sourceKind = isNearChain(params.fromChain) ? 'near' : 'evm';
            const targetKind = isNearChain(params.toChain) ? 'near' : 'evm';
            ...
            const quoteResult = await completeQuote({
                ...
                dexRouters: buildQuoteRouters(sourceKind, Number(params.fromChain), params.publicClient || params.walletClient),
                bluechipTokens,
                configAdapter: buildConfigAdapter(),
                currentUserAddress,
            });
            ...
            const rate = Number(formattedInputAmount) > 0
            ...
            return {
                ...
                outputUsd,
                rate,
                timeEstimate: quoteResult.preSwap?.quote?.timeEstimate || 0,
                priceImpact,
                gasFeeUsd: 0,
    ...
    async executePreSwap(quote, walletClient, nearWallet) {
        ...
        const routers = buildExecutionRouters(rawQuote.sourceKind, rawQuote.fromChainId, walletClient, nearWallet);
        const routeType = preSwap.routeType || (rawQuote.sourceKind === 'evm' ? 'evm' : 'v2');
        const router = routeType === 'evm' || routeType === 'v1'
            ? routers[0]
            : routers[1] || routers[0];
        if (!router) {
        ...
        const result = await router.executeSwap(executeParams, walletClient, nearWallet);
    ...
    async getTransactionStatus(p) {
        try {
            ...
            const status = mapExecutionStatus(res.status);
            const txHash = res.swapDetails?.destinationChainTxHashes?.[0]?.hash || p.sourceTxHash || '';
            return {
                txHash,
                status,
            };
...
//# sourceMappingURL=RheaAdapter.js.map