...
import { Button, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, List, useTheme, } from '@mui/material';
...
const DeleteIconButton = ({ onClick }) => {
    ...
    return (_jsx(IconButton, { size: "medium", edge: theme?.navigation?.edge ? 'end' : false, onClick: onClick, children: _jsx(DeleteOutline, {}) }));
};
export const ActiveTransactionsPage = () => {
    ...
    const headerAction = useMemo(() => executingRoutes.length ? (_jsx(DeleteIconButton, { onClick: toggleDialog })) : undefined, [executingRoutes.length, toggleDialog]);
