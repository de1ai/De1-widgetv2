...
export const TokenFallback = ({ token, isLoading, ...other }) => {
    ...
    return (_jsx(TokenBase, { token: {
            ...token,
            priceUSD: token.priceUSD || chainToken?.priceUSD,
            logoURI: token.logoURI || chainToken?.logoURI,
        }, isLoading: isLoading || isLoadingToken, ...other }));
};
export const TokenBase = ({ token, impactToken, enableImpactTokenTooltip, step, stepVisible, disableDescription, isLoading, ...other }) => {
    ...
    return (_jsxs(Box, { ...other, sx: [
        ...
        ], children: [_jsx(TokenAvatar, { token: token, chain: chain, isLoading: isLoading, sx: { marginRight: 2 } }), _jsxs(Box, { sx: {
                    flex: 1,
                            ...
                            display: 'flex',
                            alignItems: 'center',
                        }, title: tokenAmount, children: _jsx(TextFitter, { height: 30, textStyle: {
                                fontWeight: 700,
                            }, children: t('format.tokenAmount', {
                                value: tokenAmount,
                            }) }) }), _jsxs(TextSecondaryContainer, { as: "span", children: [_jsx(TextSecondary, { children: t('format.currency', {
                                    value: tokenPrice,
                                }) }), impactToken ? (_jsx(TextSecondary, { px: 0.5, dot: true, children: "\u2022" })) : null, impactToken ? (enableImpactTokenTooltip ? (_jsx(Tooltip, { title: t('tooltip.priceImpact'), sx: { cursor: 'help' }, children: _jsx(TextSecondary, { children: t('format.percent', { value: priceImpact }) }) })) : (_jsx(TextSecondary, { title: priceImpactPercent?.toLocaleString(i18n.language, {
                                    maximumFractionDigits: 9,
                                }), children: t('format.percent', { value: priceImpact }) }))) : null, !disableDescription ? (_jsx(TextSecondary, { px: 0.5, dot: true, children: "\u2022" })) : null, !disableDescription && step ? (_jsx(TokenStep, { step: step, stepVisible: stepVisible, disableDescription: disableDescription, children: tokenOnChain })) : (tokenOnChain)] })] })] }));
};
