...
export const StepActions = ({ step, dense, ...other }) => {
    ...
    return (_jsxs(Box, { ...other, children: [_jsxs(Box, { sx: {
                    display: 'flex',
                    alignItems: 'center',
                }, children: [_jsx(Badge, { overlap: "circular", anchorOrigin: { vertical: 'bottom', horizontal: 'right' }, badgeContent: _jsx(SmallAvatar, { src: De1ToolLogo }), children: _jsx(StepAvatar, { variant: "circular", src: toolDetails.logoURI ?? De1ToolLogo, alt: toolDetails.name, children: toolDetails.name[0] }) }), _jsx(Box, { sx: {
                            flex: 1,
                                ...
                                lineHeight: 1.3334,
                                ml: 2,
                            }, children: toolDetails.name?.includes('De¹ Exchange')
                                ? toolDetails.name
                                : t('main.stepDetails', {
...
export const ProtocolStepDetailsLabel = ({ step, feeConfig, relayerSupport }) => {
    const { t } = useTranslation();
