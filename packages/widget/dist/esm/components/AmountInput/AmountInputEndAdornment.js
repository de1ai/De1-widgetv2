...
export const AmountInputEndAdornment = ({ formType }) => {
    ...
    const [fromChainId, fromTokenAddress] = useFieldValues(FormKeyHelper.getChainKey(formType), FormKeyHelper.getTokenKey(formType));
    const [toChainId, toTokenAddress] = useFieldValues(FormKeyHelper.getChainKey('to'), FormKeyHelper.getTokenKey('to'));
    const { token, isLoading } = useTokenAddressBalance(fromChainId, fromTokenAddress);
    ...
    const { data: prependedOperatingExpenseCostOne, isLoading: isLoadingPrependedOperatingExpenseCost, } = useQuery({
        queryKey: [
        ...
        queryFn: async () => {
            ...
            if (asUsd < 5) {
                srcChainTokenInAmount = BigInt(Number.parseInt((5 / Number(token.priceUSD)) * 10 ** token.decimals + ''));
            }
    ...
    return (_jsx(InputAdornment, { position: "end", children: isLoading && fromTokenAddress ? (_jsx(MaxButtonSkeleton, { variant: "rectangular" })) : formType === 'from' && token?.amount ? (_jsx(MaxButton, { onClick: handleMax, disabled: isMaxLoading, startIcon: isMaxLoading ? _jsx(CircularProgress, { size: 14, color: "inherit" }) : null, children: t('button.max') })) : null }));
};
